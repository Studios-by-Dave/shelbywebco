      ...
      {isOpen && (
        <div className="absolute bottom-20 right-0 w-80 bg-brand-dark border border-brand-blue/40 rounded-xl shadow-[0_0_20px_rgba(59,130,246,0.15)] mb-4">
          ...
          <div className="p-4 h-64 overflow-y-auto">
            <div className="bg-brand-darkest border border-brand-blue/20 rounded-lg p-3 mb-3">
              <p className="text-sm text-brand-cyan/80">
                Hi! I'm ShelbyBot. Click here to start a conversation with our team!
              </p>
            </div>
          </div>
          <form onSubmit={handleSubmit} className="p-4 border-t border-brand-blue/20">
            <input
              type="text"
              value={message}
              onChange={(e) => setMessage(e.target.value)}
              placeholder="Type your message..."
              className="w-full bg-brand-darkest border border-brand-blue/40 rounded-lg px-4 py-2 text-white placeholder-brand-cyan/50 focus:outline-none focus:border-brand-cyan"
            />
            <button
              type="submit"
              className="mt-2 w-full bg-gradient-to-r from-brand-blue to-brand-cyan text-white py-2 rounded-lg font-medium hover:shadow-[0_0_20px_rgba(34,211,238,0.3)] transition-all duration-300"
            >
              Send
            </button>
          </form>
